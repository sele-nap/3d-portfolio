...
function createBackTexture(): THREE.CanvasTexture {
  ...
  bg.addColorStop(0.4, '#130f20');
  bg.addColorStop(0.8, '#0a0810');
  bg.addColorStop(1, '#06050a');
  ctx.fillStyle = bg;
  ctx.fillRect(0, 0, W, H);

  ctx.save();
  ctx.strokeStyle = 'rgba(107,77,122,0.07)';
  ctx.lineWidth = 0.8;
  ...
  ctx.strokeStyle = '#6b4d7a';
  ctx.lineWidth = 5;
  ctx.strokeRect(10, 10, W - 20, H - 20);
  ctx.strokeStyle = '#d4a574';
  ctx.lineWidth = 2;
  ctx.strokeRect(42, 42, W - 84, H - 84);
  ctx.strokeStyle = 'rgba(107,77,122,0.4)';
  ctx.lineWidth = 1;
  ...
  cPos.forEach(([x, y, sx, sy]) =>
    drawOrnamentalCorner(ctx, x, y, sx, sy, '#d4a574'),
  );
...
function drawAboutIllustration(
  ...
  ctx.beginPath();
  ctx.arc(cx + 36, cy - 16, 74, 0, Math.PI * 2);
  ctx.fill();
  ctx.save();
...
function drawFormationsIllustration(
  ...
  ctx.save();
  pathHourglass();
  ctx.fillStyle = 'rgba(16, 10, 4, 0.9)';
  ctx.fill();
  ctx.restore();
...
// Strategy map: card id → illustration drawer
...
function createFrontTexture(card: CardDef): THREE.CanvasTexture {
  ...
  if (card.id === 'about') {
    ...
    bg.addColorStop(0, '#101e14');
    bg.addColorStop(0.55, '#0a1410');
    bg.addColorStop(1, '#080a08');
  } else {
    bg.addColorStop(0, '#201410');
    bg.addColorStop(0.55, '#150d08');
